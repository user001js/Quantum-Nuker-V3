...
async def help(ctx):
    ...
    embed.add_field(name=f"Enviar mensagem de texto no privado de todos os membros do serv.", value=f"`{p}5`, `{p}dme`, `{p}mdall`, `{p}dmall`, `{p}dma` ou `{p}dmeveryone` + `<mensagem>`", inline=False)
    embed.add_field(name=f"Enviar mensagem(ns) em todos os canais do servidor", value=f"`{p}6`, `{p}sa`, `{p}sac`, `{p}spamallchannels` ou `{p}scall` + `<quantidade de mensagens> <texto>`", inline=False)
    embed.add_field(name=f"Enviar mensagem(ns) no canal atual", value=f"`{p}7`, `{p}sc`, `{p}spamchannels`, `{p}spamchannel`, `{p}stc` ou `{p}scc` + `<quantidade de mensagens> <texto>`", inline=False)
    embed.add_field(name=f"Deletar todos os canais do servidor", value=f"`{p}8`, `{p}dch`, `{p}dac`, `{p}deleteallchannels` ou `{p}delallchannels`", inline=False)
    embed.add_field(name=f"Deletar todos os cargos do servidor", value=f"`{p}9`, `{p}dr`, `{p}delallroles`, `{p}dar`, `{p}deleteroles`, `{p}delroles` ou `{p}deleteallroles`", inline=False)
    embed.add_field(name=f"Criar canais no servidor", value=f"`{p}10`, `{p}sch`, `{p}spamcurrentchannel`, `{p}createchannels` ou `{p}criarcanais` + `<quantidade> <nome dos canais>`", inline=False)
    embed.add_field(name=f"Criar cargos no servidor", value=f"`{p}11`, `{p}sr`, `{p}spamroles`, `{p}swr`, `{p}criarcargos`, `{p}createroles` ou `{p}cr` + `<quantidade> <nome dos cargos>`", inline=False)
    embed.add_field(name=f"Editar ícone do servidor", value=f"`{p}12`, `{p}si`, `{p}editservericon`, `{p}editicon`, `{p}iconserver`, `{p}edi` + `<anexo/link/arquivo`", inline=False)
    embed.add_field(name=f"Editar nome do servidor", value=f"`{p}13`, `{p}sn`, `{p}edm`, `{p}editservername`, `{p}servername`, `{p}editarnomedoservidor` + `<nome>`", inline=False)
