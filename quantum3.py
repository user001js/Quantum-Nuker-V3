...
embed=discord.Embed(title="Ajuda e informações — Quantum Nuker", url="https://github.com/user001js/Quantum-Nuker-V3", description="\Baixe o **Quantum Nuker V3** __[aqui](https://github.com/user001js/Quantum-Nuker-V3)__.", color=0x0400ff)
embed.set_author(name="Quantum Nuker V3", url="https://github.com/user001js/Quantum-Nuker-V3")
embed.add_field(name="Banir todos os membros do servidor", value="`{p}2`, `{p}be`, `{p}baneveryone`, `{p}banall`, `{p}banirtodos` ou `{p}ba`", inline=False)
embed.add_field(name="Expulsar todos os membros do servidor", value="`{p}3`, `{p}ke`, `{p}kickeveryone`, `{p}expulsartodos`, `{p}kickall` ou `{p}ka`", inline=False)
embed.add_field(name="Renomear todos os membros do servidor", value="`{p}4`, `{p}rn`, `{p}rnall`, `{p}rmall`, `{p}renameallmembers` ou `{p}renomeartodos` + <novo nome de usuário>`", inline=False)
embed.add_field(name="Enviar mensagem de texto no privado de todos os membros do serv.", value="`{p}5`, `{p}dme`, `{p}mdall`, `{p}dmall`, `{p}dma` ou `{p}dmeveryone` + `<mensagem>`", inline=False)
embed.add_field(name="Enviar mensagem(ns) em todos os canais do servidor", value="`{p}6`, `{p}sa`, `{p}sac`, `{p}spamallchannels` ou `{p}scall` + `<quantidade de mensagens> <texto>`", inline=False)
embed.add_field(name="Enviar mensagem(ns) no canal atual", value="`{p}7`, `{p}sc`, `{p}spamchannels`, `{p}spamchannel`, `{p}stc` ou `{p}scc` + `<quantidade de mensagens> <texto>`", inline=False)
embed.add_field(name="Deletar todos os canais do servidor", value="`{p}8`, `{p}dch`, `{p}dac`, `{p}deleteallchannels` ou `{p}delallchannels`", inline=False)
embed.add_field(name="Deletar todos os cargos do servidor", value="`{p}9`, `{p}dr`, `{p}delallroles`, `{p}dar`, `{p}deleteroles`, `{p}delroles` ou `{p}deleteallroles`", inline=False)
embed.add_field(name="Criar canais no servidor", value="`{p}10`, `{p}sch`, `{p}spamcurrentchannel`, `{p}createchannels` ou `{p}criarcanais` + `<quantidade> <nome dos canais>`", inline=False)
embed.add_field(name="Criar cargos no servidor", value="`{p}11`, `{p}sr`, `{p}spamroles`, `{p}swr`, `{p}criarcargos`, `{p}createroles` ou `{p}cr` + `<quantidade> <nome dos cargos>`", inline=False)
embed.add_field(name="Editar ícone do servidor", value="`{p}12`, `{p}si`, `{p}editservericon`, `{p}editicon`, `{p}iconserver`, `{p}edi` + `<anexo/link/arquivo`", inline=False)
embed.add_field(name="Editar nome do servidor", value="`{p}13`, `{p}sn`, `{p}edm`, `{p}editservername`, `{p}servername`, `{p}editarnomedoservidor` + `<nome>`", inline=False)
embed.add_field(name="Obter cargo com permissões administrativas", value="`{p}14`, `{p}ga`, `{p}getadmin`, `{p}obteradmin`, `{p}conseguiradmin` ou `{p}admin` + `<nome do cargo>`", inline=False)
embed.add_field(name="Recuperar (somente em mensagem direta)", value="Cria 1 canal de texto no servidor (use caso você tenha deletado todos os canais no servidor).\nUse `{p}15 <ID do servidor>`, `{p}rg <ID do servidor>`, `{p}recuperarservidor <ID do servidor>`, `{p}rs <ID do servidor>` ou `{p}recuperateserver <ID do servidor>`", inline=False)
embed.add_field(name="Configurações", value="`{p}settings` ou `{p}config`", inline=False)
embed.add_field(name="Informações de desenvolvimento — Quantum Nuker", value="**Quantum Nuker V3**\nDesenvolvido por <@984580784956510228>, com o [MI-1](https://discord.gg/2AkAeZD6BU)\nVersão: {version} {checkVersion()}\nProjeto: https://github.com/user001js/Quantum-Nuker-V3\nServidor do Discord: https://discord.gg/2AkAeZD6BU\n> **Lista de comandos**\nEncontre os comandos e mais informações clicando __[aqui](https://github.com/user001js/Quantum-Nuker-V3/)__.", inline=False)
